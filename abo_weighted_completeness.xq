...
declare function local:calculate-score($element as item()?) as xs:decimal {
		...
		if (local-name($element) eq 'field' and $element/@element eq 'contributor' and $element/@qualifier eq 'author') then
			...
			1
    else if (local-name($element) eq 'field' and $element/@element eq 'type') then
			0.5
    else if (local-name($element) eq 'field' and $element/@element eq 'type' and $element/@qualifier eq 'dcmitype') then
			0.5
    ...
    0.5
			...
			0.5 
    else if (local-name($element) eq 'field' and $element/@element eq 'ths' or $element/@element eq 'opn') then
    0.5
...
<scores>{
  ...
  <ontaso>{data($record/*:field[@element eq 'type'][1]/@value)}</ontaso>
  <tallennuspvm>{data($record/*:field[@element eq 'date' and @qualifier eq 'available'[1]]/@value)}</tallennuspvm>
  ...
  local:calculate-score($record/*:field[@element eq 'type'][1]),
  local:calculate-score($record/*:field[@element eq 'type' and @qualifier eq 'dcmitype'][1]),
  ...
  local:calculate-score($record/*:field[@element eq 'ths'[1] or @element eq 'opn'[1]][1]),
