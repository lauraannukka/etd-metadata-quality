(:Tämä vielä testattava testiaineiston kanssa:)
...
if (
      ...
      local-name($element) eq 'field' and $element/@element eq 'description' and $element/@qualifier eq 'abstract'       
    )
    then
    1
    else if (local-name($element) eq 'subject' and not($element/@qualifier)) then
    1
...
<scores>{
  ...
  local:calculate-score($record/*:field[@element eq 'date' and @qualifier eq 'available']),
