...
declare function local:calculate-score($element as element()?) as xs:decimal {
      ...
      local-name($element) eq 'description' and $element[not(functx:has-empty-content(.))])
    )
		then
			1
    else if((local-name($element) eq 'language' or local-name($element) eq 'date' or local-name($element) eq 'contributor' or local-name($element) eq 'rights' or local-name($element) eq 'publisher' or local-name($element) eq 'format' and $element[not(functx:has-empty-content(.))]) ) then 0.5
...
declare function local:check-if-dcmitype($element as element()?) as xs:integer {
...
let $scores:=
<scores>{
  for $record in /records/Record/metadata/RDF/Publication
  let $ohjaaja:=<ohjaaja>{concat($record/ths,$record/opn)}</ohjaaja>
  let $types:=
    <types>{
