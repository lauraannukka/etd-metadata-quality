...
declare function local:calculate-score($element as element()?) as xs:integer {
		...
		if (local-name($element) eq 'field' and $element/@element eq 'contributor' and $element/@qualifier eq 'author') then
			...
			1  (:ilmoittaa hyväksymisajan:)
    else if (local-name($element) eq 'field' and $element/@element eq 'contributor' and not($element/@qualifier)) then
			1 (:tekopaikka:)
    else if(local-name($element) eq 'field' and $element/@element eq 'date' and $element/@qualifier eq 'accessioned') then
    1
			...
			1 
    else if (local-name($element) eq 'field' and $element/@element eq 'ths' or $element/@element eq 'rev') then
    1
...
<scores>{
  ...
  local:calculate-score($record/field[@element eq 'ths'[1] or @element eq 'rev'[1]][1]),
  ...
  local:calculate-score($record/field[@element eq 'date' and @qualifier eq 'accessioned'][1]),
