...
declare function local:calculate-score($element as element()?) as xs:integer {
		...
		if ((
			...
			local-name($element) eq 'date' or
			local-name($element) eq 'contributor' or
			local-name($element) eq 'description' or
			local-name($element) eq 'subject' or
      local-name($element) eq 'rights' or
      local-name($element) eq 'publisher' or
      local-name($element) eq 'format' and $element[not(functx:has-empty-content(.))] )
    )
		then
			1
...
<scores>{
  ...
  <ontaso1>{data($record/type[1])}</ontaso1>
