...
declare function local:calculate-score($element as element()?) as xs:numeric {
      ...
      local-name($element) eq 'type' or local-name($element) eq 'date.created' or local-name($element) eq 'rights' or local-name($element) eq 'publisher') and $element[not(functx:has-empty-content(.))] ) then
     0.5
  	else (0)
};
...
<scores>{
  ...
  local:calculate-score($record/date.created[1]))) div 10,2)
